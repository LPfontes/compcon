<template>
  <selector
    :title="$t('pilotSheet.narrative.skillTriggersTitle')"
      ...
      <v-card
        ...
        v-text="
          $t('common.selectors.remaining', {
            n: pilot.SkillsController.MaxSkillPoints - pilot.SkillsController.CurrentSkillPoints,
          })
        "
      />
    ...
    <template #right-column>
      ...
      >
        <div
          ...
          class="text-cc-overline"
        >
          {{ $t('common.selectors.abilityLabel') }}
        </div>
        <div
          v-if="mobile"
      ...
      jumpItems() {
        return [
          ...this.pilot.SkillsController.Skills.map(x => ({
            ...
            value: x.Skill.ID,
            subtitle: `// ${this.$t('common.selectors.pilotRank', { rank: x.Rank })} (+${x.Bonus})`,
          })),
          ...this.flatSkills
