<template>
  ...
  <selector
    :title="$t('pilotSheet.tactical.coreBonusesTitle')"
      ...
      <v-card
        ...
        v-text="
          $t('common.selectors.remaining', {
            n: pilot.CoreBonusController.MaxCBPoints - pilot.CoreBonusController.CurrentCBPoints,
          })
        "
      />
  ...
  export default {
    ...
    methods: {
      requirement(m: Manufacturer): string {
        ...
        if (m.ID === 'GMS')
          return this.$t('pilotSheet.tactical.cbRequirements.gms', {
            count: this.selectedCount(m.ID),
            abbr,
            name,
          })
        const lvl = `<b>${this.pilot.LicenseController.LicenseLevel(m.ID)}</b>`
        let output = this.$t('pilotSheet.tactical.cbRequirements.standard', {
          lvl,
          abbr,
          br,
          avail: this.availableCount(m.ID),
          selected: this.selectedCount(m.ID),
        })
        let remain = (3 % this.pilot.Level || 3) - this.pilot.LicenseController.LicenseLevel(m.ID)
        if (remain < 1) remain += 3
        if (this.pilot.Level < 12)
          output += this.$t('pilotSheet.tactical.cbRequirements.nextAvailable', {
            prefix: this.pilot.Level < 3 ? 'First' : 'Next',
            name,
            remain,
            suffix: remain === 1 ? '' : 's',
          })
