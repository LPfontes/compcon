    ...
    <div v-for="(d, index) in deployables" :key="`deployable-${index}`" style="border: 1px solid rgba(0, 0, 0, 0.2); border-radius: 3px">
      ...
      <v-row justify="center" dense class="text-center">
        ...
        <v-col v-if="(d as any).edef" cols="auto">
          <div v-html-safe="`<b>E-Defense:</b> ${(d as any).edef}`" class="caption" />
        </v-col>
        ...
        <v-col v-if="(d as any).sensor" cols="auto">
          <div v-html-safe="`<b>Sensor Range:</b> ${(d as any).sensor}`" class="caption" />
        </v-col>
        <v-col v-if="(d as any).tech_attack" cols="auto">
          <div v-html-safe="`<b>Tech Attack:</b> ${(d as any).tech_attack}`" class="caption" />
        </v-col>
